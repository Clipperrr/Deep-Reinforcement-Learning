...
for e in tqdm.tqdm(range(episodes)):
    ...
    while not truncated:
        ...
        goal_delta = obs['desired_goal'] - obs['achieved_goal']
        next_state = np.concatenate([obs['observation'][:3], obs_next['achieved_goal'] - obs_next['desired_goal']])
...
df.to_csv('PPO_FetchReach1.csv', index=False)
df = pd.DataFrame(list(zip(actor_loss_list, q_loss_list)), columns=['actor_loss', 'q_loss'])
df.to_csv('PPO_FetchReach_loss1.csv', index=False)
